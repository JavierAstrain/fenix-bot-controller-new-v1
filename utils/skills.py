...
# ---- Helpers de normalización (pandas) ----
...
def _build_mb(df):
    ...
    out["fecha_recepcion"] = _parse_date_col(df.get(find_col(df, ["fecha recep","f. recep","recepcion","fecha de recep"])))
    out["fecha_entrega"]   = _parse_date_col(df.get(find_col(df, ["fecha entrega","f. entre","entrega","fecha de entrega"])))
    ...
    # Derivados
    out["entregado_bool"] = (out["estado_entrega"].astype(str).str.lower().str.contains("entreg")) | \
    ...
    out["id"] = out["patente"].replace("", np.nan).fillna(out["ot"])  # ID único visible primero
    return out
...
def skill_facturas_por_pagar(df_fin_raw, horizonte_dias=7, proveedor=None):
    ...
    out = out[(out["vencimiento"] >= pd.Timestamp.today()) & (out["vencimiento"] <= pd.Timestamp.today()+pd.Timedelta(days=int(horizonte_dias)))]
    if proveedor:
...
def skill_entregas_proximos_dias_sin_factura(df_raw, horizonte_dias:int=7):
    df = _build_mb(df_raw)
    out = df[(df["entregado_bool"]==True) & (df["no_facturado_bool"]==True)].copy()
    limite = pd.Timestamp.today() + pd.Timedelta(days=int(horizonte_dias))
    out = out[(out["fecha_entrega"] >= pd.Timestamp.today()) & (out["fecha_entrega"] <= limite)]
...
def skill_sin_aprobacion(df_raw):
    df = _build_mb(df_raw)
    out = df[(df["entregado_bool"]==False) & (df["no_facturado_bo]()_]()]()
