...
def _build_mb(df: pd.DataFrame) -> pd.DataFrame:
    ...
    MB["FACTURADO_FLAG"]    = _get(df, "FACTURADO", "facturado_flag")
    ...
    MB["entregado_bool"] = estado_norm.str.contains("entreg", na=False) | (~MB["FECHA_ENTREGA"].isna())

    truthy = {"si","sí","si.","facturado","facturada","emitida","emitido","con factura","ok"}
    MB["facturado_bool"]   = fact_norm.isin(truthy) | (~MB["NUMERO_FACTURA"].astype(str).str.strip().eq("")) | (~MB["FECHA_FACTURACION"].isna())
    MB["no_facturado_bool"] = ~MB["facturado_bool"]
...
def skill_entregados_sin_factura(df_raw, **f):
    ...
    if v:=f.get("asesor") and "ASESOR" in t.columns: t = t[ilike(t["ASESOR"], v)]
    ...
    cols = [c for c in ["id","NOMBRE_CLIENTE","FECHA_ENTREGA","NUMERO_FACTURA","FECHA_FACTURACION","MONTO_NETO","NUMERO_DIAS_EN_PLANTA"] if c in t.columns]
    t = t[cols].sort_values("FECHA_ENTREGA", ascending=False).head(200)
...
def skill_entregados_facturados(df_raw, **f):
    ...
    if v:=f.get("desde"):        t = t[t["FECHA_ENTREGA"]>=pd.to_datetime(v, errors="coerce")]
    if v:=f.get("hasta"):        t = t[t["FECHA_ENTREGA"]<=pd.to_datetime(v, errors="coerce")]
    cols = [c for c in ["id","NOMBRE_CLIENTE","NUMERO_FACTURA","FECHA_FACTURACION","FECHA_ENTREGA","MONTO_NETO","NUMERO_DIAS_EN_PLANTA"] if c in t.columns]
    t = t[cols].sort_values("FECHA_ENTREGA", ascending=False).head(200)
    return _with_id_first(t), None
...
def skill_top_en_taller(df_raw, topn=10, **f):
    ...
    if v:=f.get("tipo_cliente"): t = t[t["TIPO_CLIENTE"].astype(str).str.lower()==str(v).lower()]
    if v:=f.get("sucursal"):     t = t[t["SUCURSAL"].astype(str).str.lower()==str(v).lower()]
    t = t[["id","NOMBRE_CLIENTE","FECHA_RECEPCION","NUMERO_DIAS_EN_PLANTA"]].sort_values("NUMERO_DIAS_EN_PLANTA", ascending=False).head(int(topn))
    return _with_id_first(t), None
...
def skill_entregas_proximos_dias_sin_factura(df_raw, horizonte_dias:int=7):
    ...
    t = t[["id","NOMBRE_CLIENTE","FECHA_ENTREGA","dias_desde_entrega"]].sort_values("FECHA_ENTREGA", ascending=True).head(200)
    return _with_id_first(t), None

def skill_sin_aprobacion(df_raw):
    MB = _build_mb(df_raw)
    t = MB[(~MB["entregado_bool"]) & (MB["no_facturado_bool"])].copy()
    t = t[["id","NOMBRE_CLIENTE","FECHA_RECEPCION","NUMERO_DIAS_EN_PLANTA"]].sort_values("NUMERO_DIAS_EN_PLANTA", ascending=False).head(200)
    return _with_id_first(t), None
